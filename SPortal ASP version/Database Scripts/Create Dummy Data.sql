...
INSERT INTO Topic (TopicName, TopicDescription, TopicBody, CreationDate, Upvotes, Downvotes, AmountOfPosts, LastPost) VALUES ('A Multi-threading Example', 'You might have wanted to execute several codes concurrently on Windows Forms, but you may encountered the Cross-Thread Operation error or faced your application in a dead mood. There are several ways of preventing the application from hanging. Here, I just want to show a simple way to beginners of how it is easy to achieve such a goal in a very straightforward way.
A couple of months ago, I wanted to start ProgressBar while at the same time doing something else within my application. So, the idea of multi-threading came up. Surely you have to separate the tasks using thread philosophy and also get the application away from hanging. I found some useful articles on the net that allowed me to complete my task successfully. Now I just want to disclose this little experience with those who have the same problem....', 'This is the body of the topic', CONVERT(DATE,GETDATE()), 0, 0, 0);
INSERT INTO Topic (TopicName, TopicDescription, TopicBody, CreationDate, Upvotes, Downvotes, AmountOfPosts, LastPost) VALUES ('A Beginner''s Tutorial for Understanding Windows Communication Foundation (WCF)', 'This article is an introduction to the Windows Communication Foundation (WCF). We will try to see the basic concepts behind WCF and will try to implement a small WCF service. We will also work out some small examples of how a WCF service can be consumed.', 'This is the body of the topic', CONVERT(DATE,GETDATE()), 0, 0, 0);
INSERT INTO Topic (TopicName, TopicDescription, TopicBody, CreationDate, Upvotes, Downvotes, AmountOfPosts, LastPost) VALUES ('Basic Introduction/Tutorial to writing sockets in C#', 'Is there any basic tutorial to writing sockets in c#? I have looked around for a while and still have not found a in-depth tutorial about how to go doing it..
Any help is appreciated.', 'This is the body of the topic', CONVERT(DATE,GETDATE()), 0, 0, 0);
